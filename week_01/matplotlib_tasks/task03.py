...
def main():
  plt.hist(life_exp2007, bins=10)
  plt.xlabel("Life Expectancy [in years]")
  plt.ylabel("Number of Countries")
  plt.title("Distribution of Life Expectancy in 2007")
  plt.show()

  # The histogram shows that most countries have life expectancy between
  # about 60 and 80 years. Only a small number of countries have very low
  # life expectancy below 50 years.

  plt.subplot(1, 2, 1)
  plt.hist(life_exp1950, bins=10)
  plt.xlabel("Life Expectancy [in years]")
  plt.ylabel("Number of Countries")
  ...
  # In 1950 most countries had life expectancy between about 30 and 50 years.
  # In 2007 the distribution improved and most countries had life expectancy between 60 and 80 years


# Creates a histogram similar to the one example in notes.md (https://github.com/SimeonHristov99/DL_25-26/blob/main/notes.md#use-cases)
def better_life_expectancy_change_histogram():
  bins = np.arange(20, 90, 5)

  hist_2007, _ = np.histogram(life_exp2007, bins=bins)
  hist_1950, _ = np.histogram(life_exp1950, bins=bins)

  y = np.arange(len(hist_2007))
  plt.barh(y, -hist_1950)
  plt.barh(y, hist_2007)

  labels = [f"{bins[i]}-{bins[i+1]}" for i in range(len(bins) - 1)]
  plt.yticks(y, labels)

  plt.xlabel("Number of Countries")
  plt.ylabel("Life Expectancy Range")
  plt.title("Life Expectancy Comparison: 1950 vs 2007")
...
if __name__ == "__main__":
  main()
  # better_life_expectancy_change_histogram()
