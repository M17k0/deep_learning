...
def calculate_loss(w1, w2, b, dataset):
  ...
  for x1, x2, y in dataset:
    y_observed = sigmoid(w1 * x1 + w2 * x2 + b)
    errors.append((y_observed - y)**2)

  return np.mean(errors)


def train_model(dataset, epochs=100000, lr=0.1, epsilon=0.001):
...
# Before sigmoid:
# - Outputs were unbounded (could be >1 or <0)
# - Values were raw linear scores, not true confidence
#
# After sigmoid:
# - Outputs are bounded in [0, 1]
# - They can be interpreted as probabilities (confidence)
# - The loss decreases more smoothly
